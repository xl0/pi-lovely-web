...
- Runtime dependency: `@xl0/pi-lovely-config` `^0.1.1`. Pi AI/coding-agent/TUI peer dependencies require `>=0.80.9`.
...
- `smart.ts`: non-agentic one-call smart-query helper and exported `SMART_QUERY_SYSTEM_PROMPT`. It resolves configured `smartQueryModel` or current Pi model/auth, limits fetched content to `smartQueryInputPercent` (75% default) while reserving configured output, prompt overhead, and 4096 safety tokens, calls `completeSimple()`, and returns model/usage/truncation details. Unknown context uses an 80K-token fallback context. The built-in grounded prompt supports summaries, extraction/comparison, troubleshooting, config/API details, security/migration notes, and verbatim excerpts. Unavailable models disable smart query for the session with a warning.
